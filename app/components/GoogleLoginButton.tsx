  ...
  return (
    <TooltipProvider>
      <Tooltip>
        ...
        <TooltipContent>Login to start uploading</TooltipContent>
      </Tooltip>
    </TooltipProvider>
  );
});
...
const GoogleLoginForm = forwardRef<
  HTMLButtonElement,
  ButtonProps & { withText?: boolean }
>(({ className, variant, size, withText, ...props }, ref) => {
  ...
  return (
    <Form className="inline" action="/auth/google" method="post">
      <Button
        className={cn(
          "font-bold shadow shadow-black/10 dark:shadow-none",
          className,
        )}
        ref={ref}
        {...props}
        variant={
          variant ? variant : theme === Theme.LIGHT ? "outline" : "ghost"
        }
      ...
      >
        ...
        <svg
          ...
          viewBox="0 0 24 24"
          className={`${withText ? "ms-1" : "p-1"} h-full`}
        >
          <path
